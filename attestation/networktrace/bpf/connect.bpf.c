...
int intercept_connect4(struct bpf_sock_addr *ctx) {
	...
	// DEBUG_LOG("connect4: CHECK tid=%d pid=%d cgroup=%llu comm=%s", tid, pid,
	//	  cgroup_id, comm);
	...
	// LOG("connect4: INTERCEPT pid=%d comm=%s dst=%x:%d", pid, comm,
	// dest_ip,
	//     dest_port);
...
int intercept_connect6(struct bpf_sock_addr *ctx) {
	...
	DEBUG_LOG("connect6: CHECK tid=%d pid=%d cgroup=%llu comm=%s", tid, pid,
		  cgroup_id, comm);
	int should_intercept_result = should_intercept(tid, cgroup_id, comm);
